...
function rowToProject(row) {
    return {
        ...
        freelancerAddress: row.freelancer_address ?? undefined,
...
export async function getProjectByContractId(contractId) {
    const pool = getPool();
    if (pool) {
        const result = await pool.query("SELECT * FROM projects WHERE contract_id = $1", [contractId]);
        const row = result.rows[0];
        if (!row)
            return undefined;
        return rowToProject(row);
